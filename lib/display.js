...
module.exports = function(PORT) {
	...
	function startDisplay() {
		...
		port.write(buf);
		return setInterval(function() { refresh(); }, INTERVAL_MS / REFRESH_INTERVAL);
	}
	...
	function refresh() {
		...
		if (INTERVAL_MS > (REFRESH_INTERVAL * scroll_count++)) {
			clearInterval(timer);
			setTimeout(function() {
				timer = startDisplay();
			}, 0);
		...
		var c = scroll_obj['msg'][2].shift(); scroll_obj['msg'][2].push(c);
		buf = Buffer.concat([buf, selectWindow(4)]);
