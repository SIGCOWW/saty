...
module.exports = function(PORT) {
	...
	function startDisplay() {
		...
		// createWindow
		buf = Buffer.concat([buf, createWindow(1, 0, 0, (16*2)*3, 16*3)]);
		buf = Buffer.concat([buf, createWindow(2, 0, 16*3, (16*2)*3, 16)]);
		if (obj['short']) {
			...
			buf = Buffer.concat([buf, createWindow(4, (16*2)*3, 64-16, 256-16*2*3, 16)]);
		}

		// for wno-1
		buf = Buffer.concat([buf, selectWindow(1)]);
		buf = Buffer.concat([buf, setSize(3, 3)]);
	...
	function refresh() {
		...
		if (scroll_count % 50 === 0) {
			...
			buf = Buffer.concat([buf, setReverse(scroll_obj['reverse'] ? 0 : 1)]);
