...
module.exports = function(IPADDR) {
	...
	this.printLabel = function(obj, callback) {
		// 1b 4a n ... 紙送り
		var device = getDevice(this.IPADDR);
		var printer = new escpos.Printer(device);
		device.open(function() {
			...
			.text('～ https://sigcoww.org/book/l0-4/ で DL', utils.ENCODING)
			.text('DLコードは本文中にあるよ添え～', utils.ENCODING)
			.print('\x1c\x21\x02')
			.text('光文1年23月45日    正化67年8月9日')
			.print('\x1c\x21\x00')
			.print('\x1c\x21\x0c\x1b\x21\x30')
			.text('500', utils.ENCODING)
			.print('\x1c\x21\x00\x1b\x21\x00')
			.text('委託先: Rosenblock Chainers、発売元: SIGCOWW', utils.ENCODING)
			.text('担: ' + obj['seller'], utils.ENCODING)
			.close();
		});
	}
}
