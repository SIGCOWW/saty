...
module.exports = function(PORT) {
	...
	function startDisplay() {
		...
		if (obj['short']) {
			...
			scroll_count = 0;
			scroll_obj['msg'][2] = ' '.repeat(8) + scroll_obj['msg'][2] + ' '.repeat(24);
			scroll_obj['reverse'] = true;
			scroll_obj['force'] = false;
		}

		port.write(buf);
		return setInterval(function() { refresh(); }, Math.ceil(INTERVAL_MS / REFRESH_INTERVAL_RATE));
	}
	...
	function refresh() {
		if (scroll_obj === null) {
			...
			timeout = setTimeout(function() {
				timer = startDisplay();
			}, Math.ceil(SHORT_INTERVAL_MS / 2));
			return;
		}

		if (scroll_count++ > REFRESH_INTERVAL_RATE) {
			clearInterval(timer);
			clearTimeout(timeout);
			timeout = setTimeout(function() {
				timer = startDisplay();
		...
		var c = scroll_obj['msg'][2][0];
		//if (utils.jpwidth(c) === 2 && !scroll_obj['force']) {
		//	scroll_obj['force'] = true;
		//} else {
			scroll_obj['force'] = false;
			scroll_obj['msg'][2] = scroll_obj['msg'][2].substring(1) + c;
			buf = Buffer.concat([buf, iconv.encode(c, utils.ENCODING)]);
			port.write(buf);
		//}

		var blink = new Buffer(0);
		if (scroll_count % 50 === 0) {
			blink = Buffer.concat([blink, selectWindow(1)]);
			//blink = Buffer.concat([blink, clear()]);
			blink = Buffer.concat([blink, setReverse(scroll_obj['reverse'] ? 0 : 1)]);
			blink = Buffer.concat([blink, iconv.encode('\n ' + scroll_obj['type'] + ' ', utils.ENCODING)]);
			blink = Buffer.concat([blink, selectWindow(4)]);
			scroll_obj['reverse'] = !scroll_obj['reverse']
		}
		port.write(blink);
	}
