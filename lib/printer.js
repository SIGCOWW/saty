...
module.exports = function(IPADDR) {
	...
	this.printLabel = function(seller) {
		...
		device.open(function() {
			...
			.print('\x1d\x42\x01')
			.print(iconv.encode('国産', utils.ENCODING))
			.print('\x1d\x42\x00')
			.print('\x1c\x21\x00\x1b\x21\x00')
			.print('  COSMIC ')
			.print('\x1c\x21\x0c\x1b\x21\x30')
			.text('L0 Vol.4', utils.ENCODING)
			.print('\x1c\x21\x00\x1b\x21\x00')
			.print('\x1b\x4a\x22')
			.text('原材料名: 巡回セールスマン問題、ESC/POS（電子棚札を含む）、同人誌CI、部分構造解析（化合物）', utils.ENCODING)
			.print('\x1b\x4a\x04')
			.text('保存方法: 直射日光を避け保存', utils.ENCODING)
			.print('\x1b\x4a\x04')
			.text('加 工 日: 2017.10.22', utils.ENCODING)
			.print('\x1b\x4a\x04')
			.text('消費期限: 2038.01.19', utils.ENCODING)
			.print('\x1b\x4a\x04')
			.text('電 子 版: https://sigcoww.org/book/', utils.ENCODING)
			.print('\x1b\x4a\x04')
			.text('DLコード: 本文中に記載', utils.ENCODING)
			.print(iconv.encode('委 託 元: SIGCOWW   お値段 ', utils.ENCODING))
			.print('\x1c\x21\x0c\x1b\x21\x38')
			.print('500')
			.print('\x1c\x21\x00\x1b\x21\x00')
			.text('円', utils.ENCODING)
			.print('\x1b\x4a\x04')
			.text('US0800担: ' + seller, utils.ENCODING)
			.print('\x1b\x4a\x44')
			.cut()
			.close();
		});
