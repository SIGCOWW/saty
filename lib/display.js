...
module.exports = function(PORT) {
	...
	function startDisplay() {
		...
		buf = Buffer.concat([buf, createWindow(2, 0, 16*2, (16*2)*2, 16)]);
		if (obj['short']) {
		...
		buf = Buffer.concat([buf, new Buffer(obj['label'].replace('COSMIC ', ''), 'ascii')]);

		// for wno-3
		var timetimetime = INTERVAL_MS;
		...
		if (obj['short']) {
			...
			buf = Buffer.concat([buf, setScroll()]);
			buf = Buffer.concat([buf, iconv.encode('　'.repeat(16), utils.ENCODING)]);
			buf = Buffer.concat([buf, new Buffer([0x1f, 0x3a])]);
			buf = Buffer.concat([buf, iconv.encode(obj['msg'][2].trim(), utils.ENCODING)]);
			buf = Buffer.concat([buf, iconv.encode('　'.repeat(16), utils.ENCODING)]);
			buf = Buffer.concat([buf, new Buffer([0x1f, 0x3a])]);
		...
		port.write(buf);
		return setInterval(function() { refresh(); }, Math.ceil(timetimetime));
	}
