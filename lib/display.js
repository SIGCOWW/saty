...
function getMessages() {
	...
	for (var i=0; i<books.length; i++) {
		...
		for (var j=0; j<data['description'].length; j++) {
			...
			copy['msg'] = [ data['description'][i]['title'] ];
			copy['msg'].push(data['description'][i]['author'] + ' 著');
			copy['msg'].push(data['description'][i]['text']);
			messages.push(copy);
		}
	}
...
function initial() {
	return new Buffer([
		...
		0x1b, 0x52, 0x08,
		0x1b, 0x74, 0x01,
		0x1f, 0x28, 0x47, 0x02, 0x00, 0x40, 0x01
	]);
}
